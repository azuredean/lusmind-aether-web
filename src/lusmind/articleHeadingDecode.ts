/*
 * Adapted from ThreeUI Community's Article Heading Decode helper.
 * Source: https://github.com/MengTo/threeui
 * License: MIT. See THIRD_PARTY_NOTICES.md.
 */
export const FLAVOR_DECODE_POOL = "#%&@$/\\<>*+=~ABCDEFGHKMNPRSTUVWXYZ0123456789";

export type FlavorDecodeOptions = {
  duration: number;
  stagger: number;
...
export function startFlavorHeadingDecode(
  root: HTMLElement,
  options: FlavorDecodeOptions,
) {
  ...
  const decodeElement = (element: HTMLElement, delay: number) => {
    ...
    const frame = (now: number) => {
      const progress = clamp(
        (now - start) / Math.max(1, options.duration),
        0,
        1,
      );
      ...
      for (const textNode of nodes) {
        ...
        const scrambleLength = Math.min(
          length - revealed,
          Math.round(options.scrambleLength),
        );

        for (let index = 0; index < scrambleLength; index += 1) {
          const original = textNode.original[revealed + index];
          output +=
            original === " " || Math.random() < options.preserveChance
              ? original
              : FLAVOR_DECODE_POOL[
                  (Math.random() * FLAVOR_DECODE_POOL.length) | 0
                ];
        }
        ...
        output += textNode.original
          ...
          .replace(/\S/g, (character) =>
            Math.random() < options.tailChance
              ? FLAVOR_DECODE_POOL[
                  (Math.random() * FLAVOR_DECODE_POOL.length) | 0
                ]
              : character,
  ...
  root
    .querySelectorAll<HTMLElement>("[data-flavor-heading]")
